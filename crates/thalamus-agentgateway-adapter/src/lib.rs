...
/// BackendPort adapter for Agentgateway's OpenAI-compatible LLM routing surface.
///
/// Translates Thalamus Envelopes into Agentgateway /v1/chat/completions requests
/// and parses responses into BackendResponses. This is the ONLY place where
/// HTTP client and provider/wire knowledge may exist for Agentgateway.
pub struct AgentgatewayAdapter {
    config: AdapterConfig,
    agent: ureq::Agent,
}

impl AgentgatewayAdapter {
    ...
    pub fn call_internal(&self, envelope: &Envelope) -> Result<BackendResponse, AdapterError> {
        ...
        request = request.header("x-trace-id", envelope.trace_id.0.to_string());
        request = request.header("x-audit-id", envelope.audit_id.0.to_string());
...
impl BackendPort for AgentgatewayAdapter {
...
mod tests {
    ...
    fn test_adapter(url: &str) -> AgentgatewayAdapter {
        AgentgatewayAdapter::new(AdapterConfig {
